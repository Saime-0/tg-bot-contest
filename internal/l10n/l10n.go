...
__` + CfgChatUsername + `__ \- обязательный, если не указан параметр "` + CfgChatID + `"\. Чат в котором будет проводиться конкурс\. Не всегда удается по этому параметру определить чат, в этом случае лучше использовать "` + CfgChatID + `"\.
__` + CfgChannelUsername + `__ \(либо __` + CfgChannelID + `__\) \- опциональный параметр\. Канал в котором будет запущен конкурс\. Участники должны будут приглашать друзей в него, а не в чат, но ключевое слово все также надо писать в чат\.
